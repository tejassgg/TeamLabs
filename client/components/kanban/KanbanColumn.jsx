...
const KanbanColumn = React.memo(({ statusCode, statusName, icon, bgColor, tasks, handleDragStart, handleDragEnd, handleDrop, isTaskAssignedToUser, isLast, addTaskButton }) => {
  ...
  return (
    <div
      className={`flex flex-col h-[900px] bg-transparent ${!isLast ? 'border-r border-gray-200 dark:border-gray-700' : ''}`}
      onDragOver={(e) => e.preventDefault()}
      onDrop={() => handleDrop(statusCode)}
    >
      <div className={`px-4 py-3 border-b ${statusStyle.borderLight} flex items-center gap-2 sticky top-0 bg-transparent dark:${statusStyle.borderDark} dark:bg-transparent`}>
        <div className={`${statusStyle.textLight} dark:${statusStyle.textDark}`}>
          {icon}
        </div>
        <h3 className={`font-semibold ${statusStyle.textLight} dark:${statusStyle.textDark}`}>
          {statusName}
        </h3>
        <span className={`ml-auto px-2 py-1 text-xs rounded-full border ${statusStyle.borderLight} dark:${statusStyle.textDark} dark:${statusStyle.borderDark}`}>
          {tasksInStatus.length}
        </span>
      </div>
      <div className={`p-3 flex-1 ${columnScrollClass} bg-transparent dark:bg-transparent`}>
