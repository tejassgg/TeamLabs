...
import { authService, commonTypeService, organizationService, teamService } from '../services/api';
...
const FirstTimeSetup = ({ isOpen, onComplete }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className={`${theme === 'dark' ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'} rounded-xl shadow-2xl max-w-6xl w-full max-h-[95vh] overflow-hidden py-10 px-10`}>
        {/* Header */}
  ...
  const handleProfileComplete = (data) => {
    ...
    if (data && data.organizationID) {
      ...
      setSelectedOrganization({
        ...
        name: data.organizationName || 'Selected Organization'
      });
    }
    if (onNext) onNext();
  };
...
const TeamDropdown = ({ value, onChange, options, placeholder = 'Select Team', required = false, disabled = false, className = '', onCreateTeam, setDropdownOpen, requestedTeams, setRequestedTeams }) => {
  const [isOpen, setIsOpen] = useState(false);
  const { theme } = useTheme();
  const [search, setSearch] = useState('');
  const dropdownRef = React.useRef(null);
...
const ProjectStep = ({ step, setupProgress, onNext, onPrevious, onSkip }) => {
  ...
  const userProjectMemberships = projects?.filter(project => {
    // Skip if project is undefined/null
    if (!project) return false;
    
    // If project has a members array, check for userDetails._id
    if (project.members && Array.isArray(project.members)) {
      return project.members.some(m => m.MemberID === userDetails?._id);
    }
    // Fallback: if project has a ProjectOwner or similar
    return project.ProjectOwner === userDetails?._id;
  }) || [];
  const isMemberOfAnyProject = userProjectMemberships.length > 0;
  ...
  const handleAddProject = async (projectData) => {
    ...
    try {
      // Add project via API
      const { project } = await require('../services/api').projectService.addProject({
        ...projectData,
        ProjectOwner: userDetails?._id,
        OrganizationID: userDetails?.organizationID
      });
      setProjects(prev => [...prev, project]);
      setProjectOptions(prev => [...prev, project]);
      setSelectedProjectId(project.ProjectID?.toString());
      ...
      <div className={`p-6 rounded-lg ${theme === 'dark' ? 'bg-gray-700' : 'bg-gray-50'}`}> 
        ...
        <div className="space-y-4">
          {isMemberOfAnyProject ? (
            <>
              <div className="mb-4">
                <div className="font-medium mb-2">You are already a member of the following project(s):</div>
                <ul className="space-y-2">
                  {userProjectMemberships.map(project => (
                    <li key={project.ProjectID} className={`p-4 rounded-lg border ${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                      <div className="font-bold text-lg">{project.Name}</div>
                      <div className="text-gray-600 text-sm">{project.Description}</div>
                      <div className="text-xs text-gray-400 mt-1">ID: {project.ProjectID}</div>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <>
              <div>
                <label className={`block text-sm font-medium mb-2 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>Select Project</label>
                <ProjectDropdown
