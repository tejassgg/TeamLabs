...
const StatusPill = ({ status, theme, showPulseOnActive = false, className = '' }) => {
  ...
  return (
    <div
      className={`inline-flex items-center gap-2 px-1.5 py-1 rounded-full text-sm font-medium ${theme === 'dark' ? 'bg-transparent border border-gray-700' : 'bg-white border border-gray-200'
        } ${className}`}
    >
