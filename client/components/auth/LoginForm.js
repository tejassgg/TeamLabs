...
const LoginForm = ({ onSuccess, onOpenRegister, onOpenForgotPassword }) => {
  ...
  const handleGoogleRedirectSuccess = async (accessToken, inviteToken = null) => {
    try {
      setIsLoading(true);
      setError('');
      const response = await googleLogin(null, inviteToken, accessToken);
      if (response.success) {
        showToast('Login Successful!', 'success');
        if (onSuccess) onSuccess();
      } else {
  ...
  const loginRedirect = useGoogleLogin({
    ...
    onError: (err) => {
      setError('Google login redirect failed');
      console.error(err);
    }
  });
  ...
  const handleMobileGoogleLogin = () => {
    if (typeof window !== 'undefined') {
      const inviteToken = router.query.invite || router.query.inviteToken;
      if (inviteToken) {
        sessionStorage.setItem('google_invite_token', inviteToken);
      }
    }
    loginRedirect();
  };
