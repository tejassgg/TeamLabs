...
const ReactionOptions = ({ onSelectReaction, isOwnMessage, theme }) => {
  ...
  const border = theme === 'dark' ? 'border-[#424242]' : 'border-gray-200';
      ...
      <div className={`flex items-center px-1 py-0.5 rounded-lg border shadow-sm ${panel} ${border}`}>
        {emojis.map((emoji) => (
          <button
            key={emoji}
            type="button"
            className={`text-md px-1 py-0.5 rounded-lg transition-colors ${theme === 'dark' ? 'hover:bg-[#424242]' : 'hover:bg-gray-100'
              }`}
            onClick={(e) => {
              e.stopPropagation();
