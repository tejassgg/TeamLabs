...
import { FaPaperPlane, FaPlus, FaSmile, FaImage, FaVideo, FaChevronDown, FaCheck, FaTimes } from 'react-icons/fa';
...
export default function MessagesPage() {
  ...
  const handleUpload = async (file) => {
    ...
    // For chat media not tied to task/project, we just upload and use returned URL
    const res = await messagingService.uploadChatMedia(form);
    const mediaUrl = res?.attachment?.FileURL || res?.url;
  ...
  return (
    <Layout>
      <div className={`flex h-[calc(100vh-120px)] ${bg}`}> 
        <aside className={`w-80 border-r ${panel} p-3 overflow-y-auto`}>
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-semibold text-lg">Messages</h2>
            <button className={`px-3 py-1 rounded-lg ${theme === 'dark' ? 'hover:bg-[#424242]' : 'hover:bg-blue-50 text-blue-600'} flex items-center gap-2`} onClick={() => setShowNewConversation(true)}>
              <FaPlus /> New
            </button>
          </div>
          <div className="space-y-2">
            {conversations.map((c) => {
              ...
              const displayName = isGroup ? (c.name || 'Group') : (c.participants?.filter(p => p._id !== user._id).map(p => `${p.firstName} ${p.lastName}`).join(', ') || 'Direct');
              let initials = 'GR';
              if (!isGroup) {
                ...
                initials = ((parts[0]?.[0] || '') + (parts[1]?.[0] || '')).toUpperCase() || 'GR';
              }
              return (
                <button key={c._id} onClick={() => setSelectedConversation(c)} className={`w-full text-left p-2 rounded-lg border flex items-center gap-3 ${selectedConversation?._id === c._id ? (theme === 'dark' ? 'bg-blue-900 text-blue-200' : 'bg-blue-50 text-blue-700') : ''} ${panel}`}>
                  {isGroup && c.avatarUrl ? (
          ...
          <div className="fixed inset-0 flex items-center justify-center bg-black/40">
            <div className={`w-full max-w-lg rounded-2xl border ${panel} p-4`}>
                            ...
                            return (
                              <button
                                ...
                                className={`w-full flex items-center justify-between px-3 py-2 text-left transition-colors duration-150 ${selected
                                  ...
                                  : theme === 'dark' ? 'hover:bg-[#2A2A2A] text-[#F3F6FA]' : 'hover:bg-gray-50 text-gray-900'}`}
                              >
              ...
              <div className="flex gap-2 justify-end">
                <button className={`px-3 py-1 rounded-lg ${theme === 'dark' ? 'hover:bg-[#424242]' : 'hover:bg-gray-100'}`} onClick={()=>{
                  setShowNewConversation(false);
                  setConversationType('dm');
                  setGroupName(''); setGroupAvatar(''); setGroupMembers([]); setSelectedDmUser(null);
                }}>Cancel</button>
                {conversationType==='dm' ? (
                  <button className={`px-3 py-1 rounded-lg ${theme === 'dark' ? 'hover:bg-[#424242]' : 'hover:bg-blue-50 text-blue-600'}`} disabled={!selectedDmUser} onClick={async ()=>{
                    try {
                  ...
                  <button className={`px-3 py-1 rounded-lg ${theme === 'dark' ? 'hover:bg-[#424242]' : 'hover:bg-blue-50 text-blue-600'}`} onClick={async ()=>{
                    try {
                      const ids = groupMembers.map(u=>u._id);
